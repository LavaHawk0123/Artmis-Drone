...
arucoDetection::arucoDetection() {
    ...
    pub_marker = nh.advertise < sensor_msgs::Image > ("/marker_detect_drone_top/image_raw", 100);					// Publisher for Image of detection
    pub_pose_estimated = nh.advertise < sensor_msgs::Image>("/marker_detect_drone_top/estimated",100);				// Publisher for Image of estimation
    pub_marker_id = nh.advertise < std_msgs::String > ("/marker_detect_drone_top/id", 100);						// Publisher for ID of detected marker
    pub_pose = nh.advertise < geometry_msgs::PoseStamped > ("marker_drone_top/pose", 1000);						// Publisher for pose of detected marker
    pub_velocity = nh.advertise < geometry_msgs::Twist > ("/cmd_vel", 1);							// Publisher for rover velocity
    viz_marker_pub = nh.advertise < visualization_msgs::Marker >("/tag_marker_drone_top",100);					// Publisher for Visual Marker of Tag
    ...
    sub_camera_sim = nh.subscribe("/drone/camera_top/image_raw", 100, &arucoDetection::imageCallback,this);		// General Callback for testing
}
